  ...
  return (
    <div className="relative min-h-screen overflow-hidden p-4 flex items-center justify-center">
      ...
      <div className="relative max-w-md w-full bg-white/10 border-4 border-white/40 shadow-[8px_8px_0_rgba(0,0,0,0.5)] p-5">
        <h2 className="text-white font-black text-sm text-center leading-loose mb-1">{t('Tu entrenador')}</h2>
        <p className="text-white/80 text-[10px] leading-loose text-center mb-4">
          {t('Así te verá tu rival en los combates Online')}
        </p>
        ...
        <div className="flex justify-center gap-6 bg-black/30 border-4 border-white/20 p-3 mb-4">
          <div className="text-center">
            ...
            <p className="text-white/60 text-[9px] mt-1">{t('de frente')}</p>
          </div>
          <div className="text-center">
            <PixelTrainer gender={gender} outfit={outfit} view="back" className="w-16 h-[5.5rem] mx-auto" />
            <p className="text-white/60 text-[9px] mt-1">{t('de espaldas')}</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Conjuntos de ropa */}
          <div>
            <p className="text-white/80 text-[10px] leading-loose mb-2">{t('Elige tu ropa')}</p>
            <div className="grid grid-cols-4 gap-2">
              {OUTFITS.map(option => (
                ...
                >
                  ...
                  <p className="text-white text-[8px] font-black text-center truncate leading-loose">{t(option.name)}</p>
                </button>
              ))}
            </div>
          </div>
